...
bool mouseMove(HANDLE hDevice, BYTE deltaX, BYTE deltaY, BYTE buttonFlags, BYTE wheelDelta) {
    // 发送数据到设备
...
int main() {
    ...
    if ((DevicePath = GetDevicePath()) != NULL)
    {
        ...
        while (TRUE)
        {
            // 显示操作菜单
            printf("\n========== 狐狸的驱动测试程序 =========\n");
            printf("1. 发送键盘输入A键2秒(不开放按下功能，否则停不下来)\n");
            printf("2. 发送鼠标移动\n");
            printf("3. 发送鼠标点击\n");
            printf("4. 鼠标滚轮测试\n");
            printf("0. 退出\n");
            ...
            case 1:
                ...
                break;
            case 2:
                if (mouseMove(hDevice, 120, 120, 120, 0)) {
                    printf("移动鼠标完成!");
                }
