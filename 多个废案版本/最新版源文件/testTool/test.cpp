...
int main() {
    ...
    while (TRUE)
    {
        ...
        wprintf(L"特殊功能：\n");
        wprintf(L"66. 查看键盘鼠标按下状态\n");
        wprintf(L"77. 清空命令行\n");

        wprintf(L"0. 退出\n");
        wprintf(L"========================================\n");
        wprintf(L"所有操作均延迟1秒后执行\n");
        wprintf(L"请选择操作: ");
        ...
        while (!validInput) {
            ...
            if (std::wcin >> choice) {
                ...
                if (choice >= 0 && choice <= 100) {
                    ...
                    wprintf(L"输入超出范围，请输入0-100之间的数字!\n");
                }
            }
            else {
                // 输入不是数字，清除错误状态并忽略无效输入
                wprintf(L"输入无效，请输入一个数字!\n");
                std::wcin.clear(); // 清除错误标志
                // 修正：正确使用std::numeric_limits获取最大值
        ...
        switch (choice) {
        case 1:
            if (typewrite("abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ 1234567890 -=_()[]{};:'\"/.,?`!@#$%^&*",50)) {
                wprintf(L"英文字符输入完成!\n");
            }
            else {
                wprintf(L"英文字符输入失败!\n");
            ...
            break;
        case 2:
            if (keyboardPressAndRelease("a", 1000)) {
                wprintf(L"长按键盘1秒完成!\n");
            }
            else {
                wprintf(L"长按键盘1秒失败!\n");
        ...
        case 3:
            if (keyboardPress("s")) {
                wprintf(L"长按键盘完成!\n");
                Sleep(5000);
                wprintf(L"不释放的后果就是一直按!已自动发送该按键抬起。如果没有抬起即将启用全部释放\n");
                keyboardRelease("s");
            }
            else {
                wprintf(L"未知的错误，强制释放所有按键\n");
                keyboardRelease(0x00);
            }
            break;
        case 4:
            if (keyboardRelease("a")) {
                wprintf(L"释放A按键完成!\n");
            }
            else {
                wprintf(L"释放A按键失败!\n");
        ...
        {
            // 在复合语句块中定义数组
            const char* args[] = { "win", "shift", "s", NULL };
            if (hotkey(args)) {
                wprintf(L"组合键完成!\n");
            }
            else {
