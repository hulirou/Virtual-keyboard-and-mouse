...
//触摸屏的设备描述符
const UCHAR mouse_touchScreen_ReportDescriptor[] = {
    // 鼠标部分 - 报告ID=2
    0x05, 0x01,        //   用法页（通用桌面控制）
    0x09, 0x02,        //   用法（鼠标）
    0xA1, 0x01,        //   集合（应用层）
...
)
{
    ...
    //初始化VHF鼠标
    VHF_CONFIG_INIT(&vhfConfig, WdfDeviceWdmGetDeviceObject(device), sizeof(mouse_touchScreen_ReportDescriptor), mouse_touchScreen_ReportDescriptor);
    //设置设备属性，并非强制
